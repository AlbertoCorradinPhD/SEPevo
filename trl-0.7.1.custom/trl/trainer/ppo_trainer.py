...
from ..models import SUPPORTED_ARCHITECTURES, PreTrainedModelWrapper, create_reference_model, AutoModelForCausalLMWithValueHead
...
class PPOTrainer(BaseTrainer):
    ...
    def batched_forward_pass(
        ...
        # Define the target device for model and inputs 
        ...
        # Move model inputs to the CPU (queries and responses stay on CPU)
        ...
        for i in range(math.ceil(bs / fbs)):
            # Slice inputs to handle mini-batches
            input_kwargs = {key: value[i * fbs : (i + 1) * fbs] for key, value in model_inputs.items()}
            query_batch = queries[i * fbs : (i + 1) * fbs]  # Remain on CPU
            response_batch = responses[i * fbs : (i + 1) * fbs]  # Remain on CPU
            
            if response_masks is not None:
                response_masks_batch = response_masks[i * fbs : (i + 1) * fbs]
            
            # Perform forward pass on the model (keep everything on the CPU)
            ...
            # Calculate log probabilities from logits (logprobs computation)
            logprobs = logprobs_from_logits(logits[:, :-1, :], input_ids[:, 1:])
            
            # Initialize masks (keep on CPU)
            masks = torch.zeros_like(attention_mask, device=device)  # Make sure the mask is on the same device as attention_mask
            masks[:, :-1] = attention_mask[:, 1:]

            # Handle mask calculation based on query length and response length
            ...
            # Move logits, values, logprobs, and masks to the CPU (if needed for computation)
            if return_logits:
                all_logits.append(logits.to(device))  # Move logits to CPU
            else:
                del logits  # Avoid memory overhead
            
            all_values.append(values.to(device))  # Move values to CPU
            all_logprobs.append(logprobs.to(device))  # Move logprobs to CPU
            all_masks.append(masks.to(device))  # Move masks to CPU

        return (
            torch.cat(all_logprobs),  # Return logprobs
            torch.cat(all_logits)[:, :-1] if return_logits else None,  # Return logits (if needed)
            torch.cat(all_values)[:, :-1],  # Return values
            torch.cat(all_masks)[:, :-1],  # Return masks
        )
