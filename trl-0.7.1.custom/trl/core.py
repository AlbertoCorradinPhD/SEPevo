...
# substitution for compatibility with transformers
...
# this function was added from trl 0.8.6
...
def logprobs_from_logits(logits, labels, gather=True):
    ...
    Ensures that logits and labels are on the same device before performing operations.
    """
    # Ensure that logits and labels are on the same device
    ...
    if labels is not None:
        ...
        logpy = torch.gather(logp, 2, labels).squeeze(-1)
    return logpy
